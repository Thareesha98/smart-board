...
const Sidebar = ({ onNavigate, activePage }) => {
  ...
  return (
    <aside className="fixed top-6 left-6 h-[calc(100vh-3rem)] w-[280px] bg-primary text-white flex flex-col rounded-[25px] shadow-custom overflow-y-auto z-50 hidden lg:flex">
      ...
      <nav className="flex-grow px-4 space-y-1">
        <h3 className="px-6 text-[10px] font-bold text-white/40 uppercase tracking-[2px] mb-2 mt-4">ADMIN NAVIGATION</h3>
        {navItems.map((item) => (
          <button
            key={item.id}
            onClick={() => onNavigate(item.id)}
            className={`w-full flex items-center gap-4 px-6 py-3 rounded-[18px] transition-all duration-300 group ${
              activePage === item.id ? 'bg-white/20 shadow-lg text-white font-bold' : 'text-white/90 hover:bg-white/10 hover:text-white'
            }`}
          >
            <i className={`fas ${item.icon} text-lg w-6 text-center ${activePage === item.id ? 'text-white' : 'text-white/60 group-hover:text-white'}`}></i>
            <span className="text-sm">{item.label}</span>
            {item.badge && <span className="ml-auto bg-accent text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{item.badge}</span>}
          </button>
        ))}
      </nav>
      ...
      <div className="mt-auto p-4 border-t border-white/10">
        ...
        <button className="w-full flex items-center gap-3 p-2 rounded-[12px] text-accent hover:bg-white/10 transition-all font-bold text-xs">
