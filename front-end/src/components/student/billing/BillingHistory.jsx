...
const HistoryItem = ({ payment, onViewReceipt, onPayNow, index }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ x: 5 }}
      className="flex flex-wrap items-center gap-4 p-4 border border-gray-200 rounded-large transition-all duration-300 hover:border-accent"
    >
      <div className="flex items-center gap-4 flex-1 min-w-[200px]">
        <div className="text-2xl font-bold text-text-dark">${payment.amount}</div>
        <div>
          <h4 className="font-semibold text-text-dark">{payment.title}</h4>
          <span className="text-sm text-text-muted">{payment.boarding}</span>
        </div>
      </div>
      ...
      {isPending ? (
        <motion.button
          ...
          className="px-4 py-2 rounded-large font-semibold text-sm bg-accent text-white hover:bg-primary transition-all duration-300"
        >
          Pay Now
        </motion.button>
      ) : (
