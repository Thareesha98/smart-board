...
const ScheduleModal = ({ isOpen, onClose, onSubmit, currentAppointmentId, getAppointmentById }) => {
 const [formData, setFormData] = useState(initialFormData);
 const isReschedule = !!currentAppointmentId;
 const existingAppointment = isReschedule ? getAppointmentById(currentAppointmentId) : null;
 const title = isReschedule ? `Reschedule Visit for ${existingAppointment?.boardingName || 'Appointment'}` : 'Schedule New Visit';

 // Effect to handle pre-filling form data on reschedule
 useEffect(() => {
  if (isReschedule && existingAppointment) {
   setFormData({
    // Pre-fill static data
    boardingId: existingAppointment.boardingId,
    boardingName: existingAppointment.boardingName,
    // Do NOT pre-fill date/time, force new selection
    visitDate: "", 
    visitTime: "",
    // Pre-fill notes if they exist
    visitNotes: existingAppointment.visitNotes || "",
   });
  } else {
   // Reset for a new schedule
   setFormData(initialFormData);
  }
 }, [isReschedule, currentAppointmentId, existingAppointment]);
 ...
 const handleSubmit = () => {
  ...
  // PASS THE ID TO THE PARENT SUBMIT HANDLER
  onSubmit(formData, currentAppointmentId); 
  onClose();
 };
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fadeIn">
   <div className="bg-card-bg p-8 rounded-large w-11/12 max-w-xl relative animate-modalSlideIn">
    ...
    <div className="space-y-4">
     {/* Boarding Select - Disabled during reschedule */}
     <div className="form-group">
      ...
      <select
       ...
       required
       disabled={isReschedule} // Disable selection on reschedule
       className="form-input disabled:bg-gray-100 disabled:text-text-muted"
      >
    ...
    <div className="flex justify-end gap-3 mt-8">
     ...
     >
      {isReschedule ? 'Confirm New Schedule' : 'Schedule Visit'}
     </button>
    </div>
